...
DEFAULT_SETTINGS: dict[str, str] = {
    ...
    "pay_intro": (
        "Оплата тарифа «{title}» — {price} ₽.\n\n"
        "Нажмите «Оплатить» — откроется защищённая форма CloudPayments. "
        "Оплата банковской картой; после оплаты доступ откроется автоматически."
    ),
    ...
    "start_ar_done": (
        ...
        "Подписка будет продлеваться автоматически по сохранённой карте."
    ),
    ...
    "sub_cancelled": (
        "Ваша подписка отменена. Доступ к закрытым ресурсам закрыт. 🔒\n"
    ...
    # Подарочная подписка
    "gift_menu_intro": (
        "🎁 Дарим подписку другу!\n\n"
        "Выберите тариф в подарок. После оплаты вы получите одноразовую ссылку — "
        "отправьте её другу, и он активирует доступ."
    ),
    "gift_intro": (
    ...
    # Приветствие нового участника в ЧАТЕ (группе). Пусто = бот молчит. {user} — тег участника.
    "chat_welcome_text": "",
...
SETTINGS_META: list[tuple[str, str, bool, str]] = [
    ...
    ("pay_intro", "Экран оплаты", True, "Плейсхолдеры: {title} {price}"),
    ...
    ("chat_welcome_text", "Приветствие нового участника в чате", True,
     "Пусто — бот ничего не пишет. {user} — упоминание (тег) нового участника."),
]
